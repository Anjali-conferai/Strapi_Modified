...
[Back to Index](README.md) | Previous: [Dos and Don'ts](08-dos-and-donts.md)
...
> **When to use:** Always set this accurately. If an event is paid but `PricingType` is set to `free`, users will not see pricing or be prompted to pay. If tickets are not configured in the Tickets and Promocodes plugin, a paid event will show **$0**. See [Payment Tickets](05-payment-tickets.md).
...
| **EventLocation** | String | Values: `"Online"`, `"In-person"`, or `"Both"` | EventLocation dropdown |
| **PricingType** | String | Determines if event is `"Free"` or `"Paid"` | PricingType dropdown |
| **EventPlatform** | String | Which platform(s) show this event (e.g., `"RKT"`) | EventPlatform field |
...
The frontend display is **dynamic and synchronized in real-time** with Strapi:

1. You **publish** an event in the Strapi dashboard.
2. The API endpoint immediately reflects the change.
3. The next time a user loads the Upcoming Events page, the frontend fetches fresh data from the API.
4. The event appears (or updates) on the website.

There is **no deploy step** or manual cache clear needed. Publishing in Strapi = live on the website.

> **This is why accuracy matters.** A typo in EventTitle, a wrong PricingType, or a missing ImageURL will show up on the live site as soon as you click Publish.
...
- [Creating Events](03-creating-events.md) -- step-by-step event creation in the dashboard
- [Payment Tickets](05-payment-tickets.md) -- setting up tickets for paid events
- [Webflow Integration](04-webflow-integration.md) -- connecting the registration form via eventUuid
- [Dos and Don'ts](08-dos-and-donts.md) -- what you can and cannot do
